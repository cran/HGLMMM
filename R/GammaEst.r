...
IWLS_Gamma<-function(Y=NULL,X=NULL,ZZ=NULL,SS=NULL,Beta=NULL,Vstart=NULL,OFFSET=NULL,Link=c("Inverse"),
            EstimateOverDisp=FALSE,LaplaceFixed=FALSE,RandDist=c("Normal"),DDR=NULL,DDY=NULL,DYgamma=NULL,DRgamma=NULL,Info=FALSE,DEBUG=FALSE,CONV=1e-4){
    ...
    Wmatgen<-function(mu,Link){
        if (Link=="Inverse")    Wvec<-mu^2
        if (Link=="Log")        Wvec<-rep(1,length(mu))
        Wvec
    }
    ...
    dWdmugen<-function(mu,Link){
        if (Link=="Inverse")    dWdmu<-2*mu
        if (Link=="Log")        dWdmu<-rep(0,length(mu))
        dWdmu
    }
    ...
    #for (iii in 1:3){
        ...
        if (DEBUG) {print("mu");print(mu)}
        #second derivatives matricies #
    
        Wvec<-Wmatgen(mu,Link)
        if (DEBUG){print("Wvec");print(Wvec)}
        WR<-list(0)
        UT<-0
        WRT<-list(0)
        for (i in 1:nrand) {
        ...
        if (LaplaceFixed==TRUE){
            ...
            for (i in 1:nrand){
                ADJ1<-rep(0,n)
                ADJ2<-rep(0,n)
                ADJ1<-t(ADJDER1[[i]])%*%(as.vector(diag(PP2[1:n,1:n]))*as.vector(1/Wvec)*as.vector(dWdmu)*as.vector(dmudeta)) # Check this here 
            ...
            if (DEBUG) {print("Corrterms");print(CorrTerms)}
                                 
            Scorr<-0.5*Phi*dmudeta*apply(CorrTerms,1,sum)
            if (DEBUG) {print("Scorr");print(Scorr)   }
         }
         ...
         # Updating Equations #
         MeanParmsLast<-c(Beta,unlist(VT))
         CPTISIGMAM<-t(TT)*rep(EISIGMAMvec,each=nrow(t(TT)))
        ...
        if (DEBUG) {print("Dvhatdlambda");print(DVhatDlambda[[i]])}
        qmod<-list(0)
        qCur<-list(0)
        qrr<-list(0)
        ...
        for (i in 1:nrand){
            ...
            if (DEBUG) {print("qCur");print(qCur[[i]])}
            ...
            qrr[[i]]<-qrr[[i]]-qCur[[i]]
            if (DEBUG) {print("qCur Final");print(qrr[[i]])}
            ...
            if (DEBUG) {print("qCur Finall");print(qrr[[i]])}
            DevianceRand[[i]]<-DevianceRand[[i]]/(1-qrr[[i]]) 
            if (DEBUG) {print("Lambda");print(Lambda);print("DevianceRand");print(DevianceRand[[i]])}
        }            
        ...
        qrrO<-qrrO+1+as.vector(2*log(Phi)/Phi)+as.vector(2*digamma(1/Phi)/Phi)
        if (DEBUG) {print("qrrO");print(qrrO)}
        # Applying the correction for the deviances #
        DevianceResp<-rep(0,n)
        
        DevianceResp<-2*(-log(Y/mu)+(Y-mu)/mu)
    ...
    DevianceRespCpy<-2*(-log(Y/mu)+(Y-mu)/mu)
