#####################################
##### Gradient Dispersion Gamma #####
#####################################

ProfileDispNormal<-function(DRgamma=NULL,Y=NULL,X=NULL,ZZ=NULL,SS=NULL,Beta=NULL,Vstart=NULL,OFFSET=NULL,Link=c("Identity"),
            EstimateOverDisp=FALSE,LaplaceFixed=FALSE,RandDist=c("Normal"),DDR=NULL,DDY=NULL,DYgamma=NULL,Info=FALSE,DEBUG=FALSE){
    ...
    Wmatgen<-function(x,Link){
        if (Link=="Inverse")    Wvec<-x^4
        if (Link=="Log")        Wvec<-x^2
        if (Link=="Identity")   Wvec<-rep(1,length(x))
        Wvec
    }
    ...
    dWdmugen<-function(x,Link){
        if (Link=="Inverse")    dWdmu<-4*(x^3)
        if (Link=="Log")        dWdmu<-2*x
        if (Link=="Identity")   dWdmu<-rep(0,length(x))
        dWdmu
    }
    ...
    #for (iii in 1:3){
        ...
        #print("left");print("Right");print(ZZ[[i]]%*%(as.vector(DVhatDlambda[[i]])*(Lambda[(1+qcum[i]):qcum[i+1]]*DDR[(1+qcum[i]):qcum[i+1],])));print
        ...
        DiagDeriv[(n+1+qcum[i]):(n+qcum[i+1]),]<-as.vector(DWRDU(U[[i]],RandDist[i])*as.vector(WR[[i]])*as.vector(DVhatDlambda[[i]])- # This is already multiplied by lambda
                                                                                as.vector(WR[[i]])*as.vector(1/Lambda[(1+qcum[i]):qcum[i+1]]))*DDR[(1+qcum[i]):qcum[i+1],]
