################################################################
# First attempt for the estimation of the Poisson distribution #
################################################################

IWLS_Poisson<-function(Y=NULL,X=NULL,ZZ=NULL,SS=NULL,Beta=NULL,Vstart=NULL,OFFSET=NULL,Link=c("Log"),
    ...
    #for (iii in 1:3){
        ...
        DevianceResp[Y!=0]<-2*(Y[Y!=0]*log(Y[Y!=0]/mu[Y!=0])-(Y[Y!=0]-mu[Y!=0]))
        DevianceResp[Y==0]<-2*mu[Y==0]
        ...
        if (DEBUG) {print("DRgamma");print(DRgamma)}
    ...
    DevianceRespCpy[Y!=0]<-2*(Y[Y!=0]*log(Y[Y!=0]/mu[Y!=0])-(Y[Y!=0]-mu[Y!=0]))
    DevianceRespCpy[Y==0]<-2*mu[Y==0]
