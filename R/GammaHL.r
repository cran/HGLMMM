########################################
##### Gamma H-likelihood component #####
########################################
#########################################################
# Likelihood objective function values and s.e. of vhat #
#########################################################
HlikeGamma<-function(Y=NULL,X=NULL,ZZ=NULL,SS=NULL,Beta=NULL,Vstart=NULL,OFFSET=NULL,EstimateOverDisp=FALSE,LaplaceFixed=FALSE,Link=c("Inverse"),
    ...
    HLikelihood<-0
    HLikelihood<-HLikelihood+sum((1/Phi)*((-Y/mu)+log(1/mu)))+sum((1/Phi)*log(1/Phi))-sum(lgamma(1/Phi))-sum((1-(1/Phi))*log(Y))
