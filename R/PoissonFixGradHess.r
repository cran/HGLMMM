################################
##### Gradient-Hessian Fix #####
################################

#library(numDeriv)

#ComputeGradHess<-function(ObjectiveFunction=NULL,...){
#    g<-grad(ObjectiveFunction,...)
#    h<-hessian(ObjectiveFunction,...)
...
PoiAdjProfV<-function(Beta=NULL,Y=NULL,B=NULL,X=NULL,ZZ=NULL,SS=NULL,OFFSET=NULL,Link=c("Log"),Vstart=NULL,EstimateOverDisp=FALSE,LaplaceFixed=FALSE,RandDist=c("Normal"),
...
#         Beta<-MeanParms[1:p]
    ...
    Likelihood<-Likelihood+sum((Y/Phi)*log(mu))-sum(mu/Phi)
