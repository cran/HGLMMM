##################################################################
##### Here we derive a gradient for fixed profile likelihood #####
##################################################################

ComputeGradHess<-function(ObjectiveFunction=NULL,...,d=0.1){
    check<-require(numDeriv)
    if (!check) stop("Install numDeriv package please")
...
GammaAdjProfV<-function(Beta=NULL,Y=NULL,B=NULL,X=NULL,ZZ=NULL,SS=NULL,OFFSET=NULL,Vstart=NULL,EstimateOverDisp=FALSE,LaplaceFixed=FALSE,RandDist=c("Normal"),Link=c("Inverse"),
...
#         Beta<-MeanParms[1:p]
    ...
    Likelihood<-Likelihood+sum((1/Phi)*((-Y/mu)+log(1/mu)))-sum((1/Phi)*log(1/Phi))-sum(lgamma(1/Phi))-sum((1-(1/Phi))*log(Y))
