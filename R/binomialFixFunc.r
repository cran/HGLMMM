########################################
##### Copied from BinGradHessNum.r #####
########################################


BinomAdjProfV<-function(Beta=NULL,Y=NULL,B=NULL,X=NULL,ZZ=NULL,SS=NULL,OFFSET=NULL,Vstart=NULL,EstimateOverDisp=FALSE,LaplaceFixed=FALSE,RandDist=c("Normal"),Link="Logit",
                            DDR=NULL,DDY=NULL,DYgamma=NULL,DRgamma=NULL,Info=FALSE,DEBUG=FALSE,RespDist=c("Binomial")){
...
#           CorrTerms<-list(0)
#            CorrTerms[[1]]<-diag(PP2[1:n,1:n])*as.vector(1/Wvec)*as.vector(1/Wvec)*as.vector((mu/B)*(B-mu))*as.vector(1-2*(mu/B))
#            for (i in 1:nrand){
#               ADJ1<-rep(0,n)
#                ADJ2<-rep(0,n)
#                ADJ1<-t(ADJDER1[[i]])%*%(as.vector(diag(PP2[1:n,1:n]))*as.vector(1/Wvec)*as.vector((mu/B)*(B-mu))*as.vector(1-2*(mu/B)))
...
#            Scorr<-0.5*((mu/B)*(B-mu))*apply(CorrTerms,1,sum) #dmudeta - change with distribution
#            print("Scorr");print(Scorr)   
#         }
         Ystar<-Y-Scorr  
         zmain<-eta+(Ystar-mu)/dmudeta
         
         PsiMstar<-PsiM ##### Change with distribution /mu*(1-mu) #####
...
#         Beta<-MeanParms[1:p]
         ...
         Convergence<-sum(abs(as.vector(VParms)-as.vector(VParmsLast)))
    ...
    if (RespDist=="Binomial") {
        Likelihood<-Likelihood+sum((Y/Phi)*log(mu/B))+sum(((B-Y)/Phi)*log(1-(mu/B))) # Add here constant of the newton symbol
    }
    ...
    Likelihood<-Likelihood-0.5*log(det(Hmat))
    Likelihood
